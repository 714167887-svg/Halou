    ...
    internal sealed partial class HalouSuiteManager
    {
        public string CurrentAssemblyPath
        {
            get
            {
                ...
                // Assembly.Load(byte[]) 时 Location 为空，回退到 _assemblyDirectory
        ...
        public bool DownloadUpdate(out string installedPath, out string errorMessage)
        {
            ...
            string targetDir = _assemblyDirectory;
            string dllPath = null;
            try
            {
                var loc = Assembly.GetExecutingAssembly().Location;
                if (!string.IsNullOrEmpty(loc))
                {
                    ...
                    if (!string.IsNullOrEmpty(d)) targetDir = d;
                }
            }
            catch { }
            ...
            {
                // Assembly.Load(byte[]) 时 Location 为空：用 payload 目录拼出当前 DLL 名
                dllPath = Path.Combine(targetDir, "JsqClipboardCadPlugin.dll");
            }
